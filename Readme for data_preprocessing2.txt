...
0-3. 1차로 위암에 관련된 유전자 888개 리스트를 뽑고
     2차로 PHGDH 와 단백질 상호작용하는 유전자 359개 리스트를 뽑은 다음,
     두 개의 리스트에서 중복되는 45개와 유전자ID를 모르는 17개를 빼서
     총 1185개의 유전자 ID 리스트를 만듦

0-4. 2번 데이터 프레임의 컬럼이름이 3번 리스트에 존재하는 것만 추출하니
     여기서 다시 28개가 빠져 최종적으로 407 * 1157 크기의 데이터프레임이 나왔고
     이를 xy_STAD-related-only2.csv 와 xy_STAD-related-only2.pkl 로 저장함
...
1-2. X, Y 값에 해당하는 컬럼 추출 (407, 1157), (407,)
...
1-5. 데이터셋을 저장 (현재는 데이터가 모자라 train + test : val = 8 : 2 로만) 
     STAD_Dataset2_Train.h5, STAD_Dataset2_Test.h5
